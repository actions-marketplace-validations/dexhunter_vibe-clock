...
これだけです。`vibe-clock setup` はエージェントを検出し、`gh` があればそのトークンを借用し、公開する JSON をそのまま提示し、Gist を作成し、リポジトリの secret を設定し、ワークフローファイルを書き込み、毎日のプッシュを登録します。**この端末の外側を変更する操作は、すべて事前に確認を求めます。** 代行できない手順については、その手順を印字します。
...
保証は一重ではなく二重です。ペイロードは [`sanitizer.py`](vibe_clock/sanitizer.py) の明示的な許可リストから組み立てられ、そこに名前のないフィールドは送れません。さらに `_validate_no_pii` が完成した JSON を再走査し、ホームディレクトリのパスかユーザー名が含まれていれば**例外を送出して**プッシュを中止します。
...
`vibe-clock export` が書き出すのは**サニタイズされていない**ローカル統計で、実際のプロジェクト名やモデル ID を含みます。ローカル分析用です。その出力を commit しないでください。
